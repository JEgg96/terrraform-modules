...
resource "aws_cloudwatch_log_group" "log_group" {
  name = "/ecs/${var.project_name}-${var.enviroment}-td"
...
resource "aws_ecs_task_definition" "ecs_task_definition" {
  family                   = "${var.project_name}-${var.enviroment}-td"
  ...
  container_definitions = jsonencode([
    {
      name      = "${var.project_name}-${var.enviroment}-container"
...
resource "aws_ecs_service" "ecs_service" {
  name                               = "${var.project_name}-${var.enviroment}-service"
  launch_type                        = "FARGATE"
  ...
  load_balancer {
    ...
    container_name   = "${var.project_name}-${var.enviroment}-container"
    container_port   = 80
  }
}