...
resource "aws_eip" "eip1" {
  ...
  tags = {
    Name = "${var.project_name}-${var.enviroment}-eip1"
  }
}
...
resource "aws_eip" "eip2" {
  ...
  tags = {
    Name = "${var.project_name}-${var.enviroment}-eip2"
  }
}
...
resource "aws_nat_gateway" "nat_gateway_az1" {
  ...
  subnet_id     = var.public_subnet_az1_id.id

  tags = {
    Name = "${var.project_name}-${var.enviroment}-ng-az1"
  }
...
resource "aws_nat_gateway" "nat_gateway_az2" {
  ...
  tags = {
    Name = "${var.project_name}-${var.enviroment}-ng-az2"
  }
...
resource "aws_route_table" "private_route_table_az1" {
  ...
  tags = {
    Name = "${var.project_name}-${var.enviroment}-private-rt-az1"
  }
}
...
resource "aws_route_table_association" "private_data_subnet_az1_rt_az1_association" {
  subnet_id      = var.private_data_subnet_az1_id.id
  route_table_id = aws_route_table.private_route_table_az1.id
}
...
resource "aws_route_table" "private_route_table_az2" {
  ...
  tags = {
    Name = "${var.project_name}-${var.enviroment}-private-rt-az2"
  }
}
